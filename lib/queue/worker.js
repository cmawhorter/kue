...
/**
 * Used for Job Sets to track set to be processed
 */
...
Worker.prototype.getJob = function(fn){
  ...
  var client = clients[self.type]
    ...
    , defKey = 'q:jobs:' + self.type + ':inactive'
    , nextJob = function(key, noretry) {
        self.zpop(key, function(err, id){
          if (err) return fn(err);
          if (!id) return noretry ? fn() : nextJob(defKey, true);
  ...
  var result = client.blpop('q:' + self.type + ':jobs', 0, function(err) {
    if (_lastJob != null && _lastJob.jobset !== undefined) 
    {
      nextJob('q:job:sets:' + self.type + ':' + _lastJob.data[_lastJob.jobset]); // if set exists, grab another similar type before moving on
    }
    else 
    {
      nextJob(defKey, true);
