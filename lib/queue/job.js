...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    ...
    if('undefined' != hash.jobset) job.jobset = hash.jobset;
    try {
 ...
 * @param {String} type
 * @param {Object} data
 * @param {String} jobset data[key]
 * @api public
 */

function Job(type, data, jobset) {
  this.type = type;
  this.data = data || {};
  this.jobset = jobset;
...
Job.prototype.state = function(state){
  ...
  else if (this.jobset) this.removeJobSet();
  return this;
};

/**
 * Remove Set entry and callback `fn(err)`.
 *
 * @param {Function} fn
 * @return {Job} for chaining
 * @api public
...
Job.prototype.removeJobSet = function(fn){
  var client = this.client;
  client.zrem('q:job:sets:' + this.type + ':' + this.data[this.jobset], this.id);
  return this;
};
...
Job.prototype.save = function(fn){
  ...
  client.incr('q:ids', function(err, id){
    ...
    if (self.jobset !== undefined)
    {
      client.zadd('q:job:sets:' + self.type + ':' + self.data[self.jobset], self._priority, id);
      self.set('jobset', self.jobset);
    }
